  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchPlayers.fulfilled, (state, action) => {
        state.loading = false;
        // If the backend returns pagination { data, total, count } (which it does based on the controller)
        if (action.payload && action.payload.data) {
          state.players = action.payload.data;
          state.totalPlayers = action.payload.total;
          // Calculate total pages if limit was passed, else assume 1
          // We don't have limit in payload directly, mostly handle it in component
        } else {
